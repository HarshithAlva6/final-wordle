...
QuartSchema(
    ...
    tags=[
        {
            "name": "Games",
            "description": "APIs for creating and playing a game for a particular user",
        },
        {
            "name": "Statistics",
...
async def _get_read_db(db_name):
    db = g._sqlite_read_db = databases.Database(app.config["DATABASES"][db_name])
    await db.connect()
    return db


async def _get_write_db():
    db = g._sqlite_write_db = databases.Database(
        app.config["DATABASES"]["PRIMARY_GAME_URL"]
    )
    await db.connect()
    return db
...
async def play_game(game_id):
    ...
    if state:
        ...
        callbacks = await get_callback_urls()
        for url in callbacks:
            q.enqueue(send_to_callback, url, leaderboard_data)
        return {
            "game_id": game_id,
            "number_of_guesses": guess_number,
...
async def get_callback_urls():
    read_db = await _get_read_db(next(iterator))
    url_list = await read_db.fetch_all("SELECT * from leaderboards")
    return [url for url, in url_list]


def send_to_callback(url, data):
    app.logger.debug(f"Sending {data} to {url}")
    r = httpx.post(url, json=data)
    if r.status_code >= 400:
        raise httpx.ConnectError("Post Unsuccessful")
